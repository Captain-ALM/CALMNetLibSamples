...
Public Partial Class msgve
    ...
    Public Function genmsgs() As Mail()
        ...
        For Each c As Reg In addrs
            msgs.Add(New Mail(0, txtbxheader.Text, txtbxdat.Text, cmarshal.internalTCPSocket(c.ip, c.port).localIPAddress, cmarshal.internalTCPSocket(c.ip, c.port).localPort, c.ip, c.port) With {.sndnom = "Me"})
        Next
        Return msgs.ToArray()
    End Function
End Class
