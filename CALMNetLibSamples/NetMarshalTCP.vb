...
Public Class NetMarshalTCP
    ...
    Public Sub New(iptb As IPAddress, ptb As Integer)
        MyBase.New(New NetTCPListener(iptb, ptb) With {.sendBufferSize = UInt16.MaxValue, .receiveBufferSize = UInt16.MaxValue, .noDelay = True, .connectionBacklog = 1})
        _myip = iptb.ToString()
        _myport = ptb
    End Sub

    Public Sub New(iptb As IPAddress, ptb As Integer, cbl As Integer)
        MyBase.New(New NetTCPListener(iptb, ptb) With {.sendBufferSize = UInt16.MaxValue, .receiveBufferSize = UInt16.MaxValue, .noDelay = True, .connectionBacklog = cbl})
        _myip = iptb.ToString()
        _myport = ptb
    End Sub

    Public Sub New(iptb As IPAddress, ptb As Integer, ipts As IPAddress, pts As Integer)
        MyBase.New(New NetTCPListener(iptb, ptb) With {.sendBufferSize = UInt16.MaxValue, .receiveBufferSize = UInt16.MaxValue, .noDelay = True, .connectionBacklog = 1})
        _myip = ipts.ToString()
        _myport = pts
    End Sub

    Public Sub New(iptb As IPAddress, ptb As Integer, cbl As Integer, ipts As IPAddress, pts As Integer)
        MyBase.New(New NetTCPListener(iptb, ptb) With {.sendBufferSize = UInt16.MaxValue, .receiveBufferSize = UInt16.MaxValue, .noDelay = True, .connectionBacklog = cbl})
        _myip = ipts.ToString()
        _myport = pts
    End Sub
    ...
    Public Overridable Sub connect(iptc As String, ptc As Integer)
        If _cl IsNot Nothing Then
            SyncLock _slockconnect
                Dim ccl As INetSocket = New NetTCPClient(IPAddress.Parse(iptc), ptc) With {.sendBufferSize = UInt16.MaxValue, .receiveBufferSize = UInt16.MaxValue, .noDelay = True}
                ccl.open()
                If ccl.connected And ccl.sendBytes(New Serializer().serializeObject(Of Message)(New Message() With {.message = Chr(6), .senderIP = _myip, .senderPort = .senderIP = _myport, .receiverIP = .receiverIP = CType(ccl, INetConfig).remoteIPAddress, .receiverPort = .senderIP = CType(ccl, INetConfig).remotePort})) Then
                    Dim acl As INetSocket = _cl.acceptClient()
                    Dim bts As Byte() = acl.recieveBytes()
                    Dim msg As Message = New Serializer().deSerializeObject(Of Message)(bts)
                ...
                While _cl IsNot Nothing AndAlso _cl.listening
                    SyncLock _slockconnect
                        If _cl.clientWaiting Then
                            ...
                            Dim bts As Byte() = acl.recieveBytes()
                            Dim msg As Message = New Serializer().deSerializeObject(Of Message)(bts)
                            Dim ccl As INetSocket = New NetTCPClient(IPAddress.Parse(msg.senderIP), msg.senderPort) With {.sendBufferSize = UInt16.MaxValue, .receiveBufferSize = UInt16.MaxValue, .noDelay = True}
                            ccl.open()
                            If ccl.connected And ccl.sendBytes(New Serializer().serializeObject(Of Message)(New Message() With {.message = Chr(6), .senderIP = _myip, .senderPort = .senderIP = _myport, .receiverIP = .receiverIP = CType(ccl, INetConfig).remoteIPAddress, .receiverPort = .senderIP = CType(ccl, INetConfig).remotePort})) Then
                                Dim clt As Thread = New Thread(New ParameterizedThreadStart(AddressOf t_cl_exec))
                                clt.IsBackground = True
                    ...
                    For i As Integer = 0 To _clcol.Count - 1 Step 1
                        Try
                            ...
                            SyncLock _slockcolman
                                ct = _clcol(i)
                            End SyncLock
                            If Not (ct.Item1.connected And ct.Item2.connected And ct.Item3.IsAlive) Then
                                ct.Item1.close()
                                ct.Item2.close()
                                SyncLock _slockcolman
                                    _clcol.Remove(ct)
